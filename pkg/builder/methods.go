...
import (
	...
	"github.com/sumup/py-sdk-gen/internal/stringx"
...
func (mt Method) ParamsString() string {
	...
	if mt.QueryParams != nil {
		res.WriteString(", ")
		res.WriteString(fmt.Sprintf("%s: %s | None = None", strcase.ToSnake(mt.QueryParams.Name), mt.QueryParams.Type))
	}
	return res.String()
}
...
func (b *Builder) getReferenceSchema(v *base.SchemaProxy) string {
	if v.GoLow().IsReference() {
		...
		isShared := slices.Contains(b.schemasByTag["shared"], v)
		if isShared {
