...
import (
	...
	"github.com/sumup/x/pointerx"
	"gopkg.in/yaml.v3"
)

func deprecationNotice(operation *v3.Operation) *string {
	if pointerx.From(operation.Deprecated) {
		if notice, ok := extension.Get[string](operation.Extensions, "x-deprecation-notice"); ok {
			return pointerx.To(notice)
		} else {
			return pointerx.To("This method is deprecated")
		}
	}
...
func writeSchemaMetainfo(out *strings.Builder, schema *base.Schema) {
	if pointerx.From(schema.ReadOnly) {
		fmt.Fprintf(out, "\nRead only")
	}
	if pointerx.From(schema.WriteOnly) {
		fmt.Fprintf(out, "\nWrite only")
	}
	...
	// arrays
	if pointerx.From(schema.UniqueItems) {
		fmt.Fprintf(out, "\nUnique items only")
	}
	...
	if pointerx.From(schema.Deprecated) {
